...
이 저장소는 공개 배포용 바이너리만 제공하는 release 레포입니다.
소스 `.py` 파일은 커밋 대상에서 제외하고, 사용자 배포에 필요한 zip 패키지만 올리는 구성을 기준으로 정리했습니다.
...
- 공개 레포에는 평문 `.py` 파일을 커밋하지 않습니다.
- `promo/`, `output/`, 로컬 설정 파일, 쿠키, 로그, 내부 서버 설정은 포함하지 않습니다.
- 실제 서버 주소는 `server_config.example.json` 예시로만 제공합니다.
